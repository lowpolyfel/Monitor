...
fixed_w, fixed_h = w, h                     # <-- tamaño fijo
cx, cy = x + w // 2, y + h // 2             # <-- centro actual
...
while True:
    ...
    if ok:
        ...
        # Si el tracker falla, conserva el último centro (cx, cy)
        pass

    # --- ROI de tamaño fijo (bloqueado) ---
    x = cx - fixed_w // 2
    y = cy - fixed_h // 2
    x = max(0, min(x, proc_w - fixed_w))
    y = max(0, min(y, proc_h - fixed_h))
    w, h = fixed_w, fixed_h

    roi = frame[y:y+h, x:x+w]
    gray_roi = gray_full[y:y+h, x:x+w]

    # --- (Re)calcular puntos si no hay suficientes o cambió el ROI ---
    need_reseed = (prev_pts is None) or (len(prev_pts) < 10)
    if need_reseed:
        feat_params = dict(maxCorners=60, qualityLevel=0.01, minDistance=4, blockSize=7)
    ...
    # Flujo óptico (ahora SIEMPRE mismo tamaño)
    p1, st, err = cv2.calcOpticalFlowPyrLK(prev_gray_roi, gray_roi, prev_pts, None, **lk_params)

    vx = vy = vmag = 0.0
    if p1 is not None and st is not None and prev_pts is not None:
        good_new = p1[st==1]
    ...
    put_text(out, "ROI punta (size fijo)", (x, max(0,y-8)), 0.6, (0,255,255), 2)
